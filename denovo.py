...
from data import *
# from novoDataLoader import DrugNovoLoader
from utils import *
...
def train(args, dataset, graph_data, cv):
    ...
    data_result = pd.DataFrame.from_dict(result, orient='index').T
...
if __name__ == '__main__':
    ...
    dataset = DiseaseNovoLoader(args.data_name, args.device,
                             symm=args.gcn_agg_norm_symm,
                             k=args.num_neighbor)
    print("col_idx:%d" % len(dataset.col_idx))
    print("Loading dataset finished ...\n")

    args.save_dir = args.data_name
    args.save_dir = os.path.join("drug_denovo", args.save_dir)
    if not os.path.isdir(args.save_dir):
        os.makedirs(args.save_dir)

    auc_list, aupr_list = [], []

    for cv in range(0, len(dataset.col_idx)):
        args.save_id = cv + 1
        print("===============" + str(cv + 1) + "=================")
